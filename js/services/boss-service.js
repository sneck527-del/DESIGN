...
(function() {
  ...
  window.renderBossView = function() {
    ...
    el.innerHTML = '<div style="display:flex;gap:8px;margin-bottom:16px;align-items:center;flex-wrap:wrap"><h2 style="font-size:16px;color:var(--text);display:flex;align-items:center;gap:6px">🔐 经营看板</h2><span style="font-size:11px;color:var(--text-dim)">当前方案: ' + (plans[S.currentPlan] || '') + '</span><div style="flex:1"></div><button class="btn btn-outline btn-sm" onclick="showDashboard()">数据看板</button><button class="btn btn-ghost btn-sm" onclick="showView(\'welcome\')">返回</button></div>' +
      '<div class="boss-stat-row"><div class="boss-stat"><div class="label">报价总额</div><div class="value">¥' + fmtInt(totalRevenue) + '</div><div class="label" style="font-size:10px;color:var(--text-dim)">共 ' + allStats.length + ' 份报价</div></div><div class="boss-stat"><div class="label">预估成本</div><div class="value cost">¥' + fmtInt(totalCost) + '</div><div class="label" style="font-size:10px;color:var(--text-dim)">成本率 ' + (rate * 100) + '%</div></div><div class="boss-stat"><div class="label">预估毛利</div><div class="value profit">¥' + fmtInt(totalProfit) + '</div><div class="label" style="font-size:10px;color:var(--text-dim)">综合毛利率 ' + totalMargin.toFixed(1) + '%</div></div></div>' +
